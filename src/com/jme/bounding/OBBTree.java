 ...
 * This class is used exclusivly by the TriMesh object.  There is no need for users to call functions on this class
 * directly.  It is a tree of OBB objects that represent a model's bound.
 * 
...
public class OBBTree{
     ...
     * @param parent The trimesh that this OBBTree should represent.
     */
     ...
     * @param start The start index of the tris array, inclusive.
     * @param end The end index of the tris array, exclusive.
     */
...
//            System.out.println("Creating right");
     ...
     * @param start Start index in the tris array, inclusive, that is the OBB to split.
     * @param end End index in the tris array, exclusive, that is the OBB to split.
     */
        ...
        if (left!=null){    // This is not a leaf
            ...
            if (collisionTree.left!=null){  // but collision isn't
                ...
                for (int i=myStart;i<myEnd;i++){
                    roti.mult(tris[i].a,tempVa).multLocal(scalei).addLocal(transi);
                    roti.mult(tris[i].b,tempVb).multLocal(scalei).addLocal(transi);
                    roti.mult(tris[i].c,tempVc).multLocal(scalei).addLocal(transi);
                    for (int j=collisionTree.myStart;j<collisionTree.myEnd;j++){
                        rotj.mult(collisionTree.tris[j].a,tempVd).multLocal(scalej).addLocal(transj);
                        rotj.mult(collisionTree.tris[j].b,tempVe).multLocal(scalej).addLocal(transj);
                        rotj.mult(collisionTree.tris[j].c,tempVf).multLocal(scalej).addLocal(transj);
     ...
     * @param collisionTree The tree to test this one against.
     * @param aList The arraylist to hold indexes of this OBBTree's triangle intersections.
     * @param bList The arraylist to hold indexes of the testing OBBTree's triangle intersections.
