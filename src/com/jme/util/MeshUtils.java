...
public class MeshUtils {
    ...
    /** */
    public static TriMesh createRectangleMesh (TriMesh rpkMesh, Vector3f rkCenter,
        Vector3f rkU, Vector3f rkV, Vector3f rkAxis,
        float fUExtent, float fVExtent, boolean bWantNormals,  boolean bWantColors,
        boolean bWantUVs)
    {
        // allocate vertices
        int iVQuantity = 4;
        ...
        {
            rpkMesh = new TriMesh("rectangle", akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createDiskMesh (TriMesh rpkMesh, int iShellSamples,
        int iRadialSamples, Vector3f rkCenter, float fRadius,
        Vector3f rkU, Vector3f rkV, Vector3f rkAxis,
        boolean bWantNormals, boolean bWantColors, boolean bWantUVs)
    {
        ...
        if ( bWantUVs )
        {
            if (akUV[0] == null) akUV[0] = new Vector2f();
            akUV[0].x = 0.5f;
            akUV[0].y = 0.5f;
        }
        ...
        for (iR = 0; iR < iRadialSamples; iR++)
        {
            ...
            for (int iS = 1; iS < iShellSamples; iS++)
            {
                ...
                if ( bWantUVs )
                {
                    if (akUV[i] == null) akUV[i] = new Vector2f();
...
//        assert( iT == iTQuantity );
        ...
        {
            rpkMesh = new TriMesh("disk", akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createBoxMesh (TriMesh rpkMesh, Vector3f rkCenter,
        Vector3f rkU, Vector3f rkV, Vector3f rkW,
        float fUExtent, float fVExtent, float fWExtent, boolean bWantNormals,
        boolean bWantColors, boolean bWantUVs, boolean bOutsideView)
    {
        // allocate vertices
        int iVQuantity = 8;
        Vector3f[] akVertex = new Vector3f[iVQuantity];
        ...
        akVertex[0] = rkCenter.subtract(kUTerm).subtractLocal(kVTerm).subtractLocal(kWTerm);
        akVertex[1] = rkCenter.add(kUTerm).subtractLocal(kVTerm).subtractLocal(kWTerm);
        akVertex[2] = rkCenter.add(kUTerm).addLocal(kVTerm).subtractLocal(kWTerm);
        akVertex[3] = rkCenter.subtract(kUTerm).addLocal(kVTerm).subtractLocal(kWTerm);
        akVertex[4] = rkCenter.subtract(kUTerm).subtractLocal(kVTerm).addLocal(kWTerm);
        akVertex[5] = rkCenter.add(kUTerm).subtractLocal(kVTerm).addLocal(kWTerm);
        akVertex[6] = rkCenter.add(kUTerm).addLocal(kVTerm).addLocal(kWTerm);
        akVertex[7] = rkCenter.subtract(kUTerm).addLocal(kVTerm).addLocal(kWTerm);
        ...
        {
            rpkMesh = new TriMesh("box",akVertex,akNormal,akColor,akUV,aiConnect);
        }
...
//            rpkMesh.updateModelNormals();
    ...
    /** */
    public static TriMesh createCylinderMesh (TriMesh rpkMesh, int iAxisSamples,
        int iRadialSamples, Vector3f rkCenter, Vector3f rkU,
        Vector3f rkV, Vector3f rkAxis, float fRadius,
        float fHeight, boolean bWantNormals, boolean bWantColors, boolean bWantUVs,
        boolean bOutsideView)
    {
        ...
        for (iA = 0, i = 0; iA < iAxisSamples; iA++)
        {
            ...
            for (iR = 0; iR < iRadialSamples; iR++)
            {
                ...
                Vector3f kNormal = rkU.mult(afCos[iR]).addLocal(rkV.mult(afSin[iR]));
        ...
        {
            rpkMesh = new TriMesh("cylinder",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createSphereMesh (TriMesh rpkMesh, int iZSamples,
        int iRadialSamples, Vector3f rkCenter, float fRadius,
        Vector3f rkU, Vector3f rkV, Vector3f rkAxis,
        boolean bWantNormals, boolean bWantColors, boolean bWantUVs, boolean bOutsideView)
    {
        ...
        for (iZ = 1, i = 0; iZ < iZSm1; iZ++)
        {
            ...
            float fSliceRadius = FastMath.sqrt(FastMath.abs(fRadius*fRadius-fZ*fZ));
            ...
            {
                float fRadialFraction = iR*fInvRS;  // in [0,1)
                Vector3f kRadial = rkU.mult(afCos[iR]).addLocal(rkV.mult(afSin[iR]));
                akVertex[i] = kSliceCenter.add(kRadial.mult(fSliceRadius));
                if ( bWantNormals )
                {
                    kNormal = akVertex[i].subtract(rkCenter);
                    kNormal.normalize();
                    if ( bOutsideView )
                        akNormal[i] = kNormal;
                    else
        ...
        // south pole
        akVertex[i] = rkCenter.subtract(rkAxis.mult(fRadius));
        if ( bWantNormals )
        {
            if ( bOutsideView )
                akNormal[i] = rkAxis.negate();
            else
                akNormal[i] = rkAxis;
        }
        if ( bWantUVs )
        ...
        // north pole
        akVertex[i] = rkCenter.add(rkAxis.mult(fRadius));
        if ( bWantNormals )
        {
            if ( bOutsideView )
                akNormal[i] = rkAxis;
...
//        assert( i == iVQuantity );
        ...
        for (i = 0; i < iRadialSamples; i++, index += 3)
        {
            if ( bOutsideView )
            {
                aiLocalConnect[index+0] = i;
                aiLocalConnect[index+1] = iVQm2;
                aiLocalConnect[index+2] = i+1;
            }
            else  // inside view
            {
                aiLocalConnect[index+0] = i;
                aiLocalConnect[index+1] = i+1;
                aiLocalConnect[index+2] = iVQm2;
            }
        }
        ...
        for (i = 0; i < iRadialSamples; i++, index += 3)
        {
            if ( bOutsideView )
            {
                aiLocalConnect[index+0] = i+iOffset;
                aiLocalConnect[index+1] = i+1+iOffset;
                aiLocalConnect[index+2] = iVQm1;
            }
            else  // inside view
            {
                aiLocalConnect[index+0] = i+iOffset;
                aiLocalConnect[index+1] = iVQm1;
                aiLocalConnect[index+2] = i+1+iOffset;
            }
        }
...
//        assert( aiLocalConnect == aiFinalConnect );
        ...
        {
            rpkMesh = new TriMesh("sphere",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createTorusMesh (TriMesh rpkMesh, int iCircleSamples,
        int iRadialSamples, Vector3f rkCenter, Vector3f rkU,
        Vector3f rkV, Vector3f rkAxis, float fOuterRadius,
        float fInnerRadius, boolean bWantNormals, boolean bWantColors, boolean bWantUVs,
        boolean bOutsideView)
    {
        // allocate vertices
        ...
        for (iC = 0, i = 0; iC < iCircleSamples; iC++)
        {
            ...
            float fTheta = FastMath.TWO_PI*fCircleFraction;
            float fCosTheta = FastMath.cos(fTheta);
            float fSinTheta = FastMath.sin(fTheta);
            Vector3f kRadial = rkU.mult(fCosTheta).addLocal(rkV.mult(fSinTheta));
            Vector3f kTorusMiddle = rkCenter.add(kRadial.mult(fOuterRadius));
            ...
            {
                float fRadialFraction = iR*fInvRS;  // in [0,1)
                float fPhi = FastMath.TWO_PI*fRadialFraction;
                float fCosPhi = FastMath.cos(fPhi);
                float fSinPhi = FastMath.sin(fPhi);
                Vector3f kNormal = kRadial.mult(fCosPhi).add(rkAxis.mult(fSinPhi));
                akVertex[i] = kTorusMiddle.add(kNormal.mult(fInnerRadius));
                if ( bWantNormals )
                {
                    if ( bOutsideView )
                        akNormal[i] = kNormal;
...
//        assert( i == iVQuantity );
        ...
        {
            rpkMesh = new TriMesh("torus",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createTetrahedronMesh (TriMesh rpkMesh, boolean bWantNormals,
        boolean bWantColors, boolean bWantUVs, boolean bOutsideView)
    {
        ...
        if ( bWantUVs )
        {
            ...
            for (i = 0; i < iVQuantity; i++)
            {
                ...
                {
                    akUV[i].x = 0.5f*(1.0f + FastMath.atan2(akVertex[i].y,
                        akVertex[i].x)*FastMath.INV_PI);
                }
                else
                {
                    akUV[i].x = 0.5f;
        ...
        {
            rpkMesh = new TriMesh("tetrahedron",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createHexahedronMesh (TriMesh rpkMesh, boolean bWantNormals,
        boolean bWantColors, boolean bWantUVs, boolean bOutsideView)
    {
        ...
        {
            rpkMesh = new TriMesh("hexahedron",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createOctahedronMesh (TriMesh rpkMesh, boolean bWantNormals,
        boolean bWantColors, boolean bWantUVs, boolean bOutsideView)
    {
        int i;
        ...
        {
            rpkMesh = new TriMesh("octahedron",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createDodecahedronMesh (TriMesh rpkMesh, boolean bWantNormals,
        boolean bWantColors, boolean bWantUVs, boolean bOutsideView)
    {
        ...
        {
            rpkMesh = new TriMesh("dodecahedron",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
    /** */
    public static TriMesh createIcosahedronMesh (TriMesh rpkMesh, boolean bWantNormals,
        boolean bWantColors, boolean bWantUVs, boolean bOutsideView)
    {
        float fGoldenRatio = 0.5f*(1.0f+FastMath.sqrt(5.0f));
        float fInvRoot = 1.0f/FastMath.sqrt(1.0f+fGoldenRatio*fGoldenRatio);
        ...
        {
            rpkMesh = new TriMesh("icosahedron",akVertex,akNormal,akColor,akUV,aiConnect);
        }
        return rpkMesh;
    }
}