/**
 * formatType.java This file was generated by XMLSpy 2006sp2 Enterprise Edition.
 * YOU SHOULD NOT MODIFY THIS FILE, BECAUSE IT WILL BE OVERWRITTEN WHEN YOU
 * RE-RUN CODE GENERATION. Refer to the XMLSpy Documentation for further
 * details. http://www.altova.com/xmlspy
 */

package com.jmex.model.collada.schema;

import com.jmex.model.collada.types.SchemaString;

public class formatType extends SchemaString {

    private static final long serialVersionUID = 1L;

    public formatType() {
        super();
    }

    public formatType(String newValue) {
        super(newValue);
        validate();
    }

    public formatType(SchemaString newValue) {
        super(newValue);
        validate();
    }

    public void validate() {
        if (value != null && length() > getMaxLength())
            throw new com.jmex.model.collada.xml.XmlException(
                    "Value of format is too long.");
    }

    public int getMaxLength() {
        return 24;
    }
}
